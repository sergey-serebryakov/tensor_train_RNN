...
class DataSet(object):
    ...
    def next_batch(self, batch_size, shuffle=True):
        ...
        if start + batch_size > self._num_examples:
            ...
            return (np.concatenate((inps_rest_part, inps_new_part), axis=0),
                    np.concatenate((outs_rest_part, outs_new_part), axis=0))
        else:
...
class DataSetS2S(object):
    def __init__(self, data, num_steps, num_test_steps=None, seed=None):
        """Construct a DataSet.
        Seed arg provides for convenient deterministic testing.
        """
        seed1, seed2 = random_seed.get_seed(seed)
        ...
        assert enc_inps.shape[0] == dec_outs.shape[0], (
                'enc_inps.shape: %s dec_outs.shape: %s' % (enc_inps.shape, dec_outs.shape))
...
def read_data_sets(data_path, s2s, n_steps, n_test_steps=None, val_size=0.1, test_size=0.1, seed=None):
